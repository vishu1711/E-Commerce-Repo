...
const navigate=useNavigate();
  ...
  const handleFilter=(value,sectionId)=>{
    ...
    let serarchParams=new URLSearchParams(location.search);

    let filterValue=serarchParams.getAll(sectionId);

    if(filterValue.length>0 && filterValue[0].split(",").includes(value)){
        filterValue=filterValue[0].split(",").filter((item)=>item!==value)

        if(filterValue.length===0){
            serarchParams.delete(sectionId);
        }
    }
    else{
    ...
    if(filterValue.length>0){
        serarchParams.set(sectionId,filterValue.join(","));

    }

const query=serarchParams.toString();
  ...
  const handleRadioFilterChange=(e, sectionId)=>{
    ...
    let serarchParams=new URLSearchParams(location.search);
     serarchParams.set(sectionId,e.target.value);
    const query=serarchParams.toString();
        navigate({search:`?${query}`})
  }
